...
%------------- BEGIN CODE --------------
function CompareMeasuresDirectories(target_dir, gen_dir)
    fprintf(">>>[INFO] Setup Paths...\n");
    targetMeasures = dir(fullfile(target_dir, '**/*measures.mat'));
    targetMeasures = targetMeasures(~[targetMeasures.isdir]);
    
    generatedMeasures = dir(fullfile(gen_dir, '**/*measures.mat'));
    ...
    for i= 1:length(generatedMeasures)
        ...
        if any(strcmp(target_names, eraseBetween(generated_names(i),1,4)))
            ...
            load(fullfile(target_dir,target_names(match_index)));
            target_measures(i) = measures;
            load(fullfile(gen_dir,generated_names(i)));
